...
import { DynamicCharacterGenerator, CharacterGenerationRequest, CharacterGenerationResult } from './dynamic-character-generator.js';
...
describe('DynamicCharacterGenerator', () => {
  ...
  describe('generateCharacter', () => {
    it('should generate fallback character when no server is provided', async () => {
      ...
      expect(result.character?.personality.signaturePhrases).toContain('我是测试角色');
    });
    ...
    it('should generate character with minimal request', async () => {
      ...
      expect(result.character?.description).toBe('角色: 简单角色');
      expect(result.character?.category).toBe('自定义角色');
    });

    it('should infer category from character name', async () => {
      const requests = [
        { name: '哆啦A梦', expected: '动漫' },
        { name: '张老师', expected: '专业人士' },
        { name: '某明星', expected: '娱乐明星' },
        { name: '王老板', expected: '商界人士' },
        { name: '未知角色', expected: '自定义角色' }
      ];

      for (const { name, expected } of requests) {
        const result = await generator.generateCharacter({ characterName: name });
    ...
    it('should handle different character types with appropriate emojis', async () => {
      ...
      for (const { name, expectedEmojis } of testCases) {
        const result = await generator.generateCharacter({ characterName: name });
        expect(result.character?.personality.emojiPreferences).toEqual(expectedEmojis);
      }
    });
  });
  ...
  describe('buildPrompt', () => {
    it('should build prompt with all fields', () => {
      ...
      expect(prompt).toContain('经典台词示例: 台词1、台词2、台词3');
    });
    ...
    it('should build prompt with minimal fields', () => {
      ...
      expect(prompt).toContain('角色: 简单角色');
      expect(prompt).not.toContain('特殊上下文');
      expect(prompt).not.toContain('经典台词示例');
    });
  });
  ...
  describe('parseLLMResponse', () => {
    it('should parse valid JSON response', () => {
      ...
      const result = (generator as any).parseLLMResponse(jsonResponse);
    ...
    it('should handle invalid JSON gracefully', () => {
      ...
      const result = (generator as any).parseLLMResponse(invalidJson);
    ...
    it('should handle partial JSON response', () => {
      ...
      const result = (generator as any).parseLLMResponse(partialJson);
  ...
  describe('extractFromText', () => {
    it('should extract personality from text response', () => {
      ...
      const result = (generator as any).extractFromText(textResponse);
    ...
    it('should handle empty text response', () => {
      const result = (generator as any).extractFromText('');
  ...
  describe('extractArrayFromLine', () => {
    it('should extract array from JSON format', () => {
      ...
      const result = (generator as any).extractArrayFromLine(line);
  ...
  describe('extractValueFromLine', () => {
    it('should extract value from key-value pair', () => {
      ...
      const result = (generator as any).extractValueFromLine(line);
  ...
  describe('extractTextFromResponse', () => {
    it('should extract text from content array', () => {
      ...
      const result = (generator as any).extractTextFromResponse(response);
    ...
    it('should return empty string for invalid response', () => {
      const result = (generator as any).extractTextFromResponse(null);
      expect(result).toBe('');

      const result2 = (generator as any).extractTextFromResponse({});
      expect(result2).toBe('');
    });
  ...
  describe('setServer', () => {
    it('should set server instance', () => {
      generator.setServer(mockServer);
      expect((generator as any).server).toBe(mockServer);
    });
  });
});