...
如果该角色没有明显的方言特点，请基于其历史背景、地区信息或人物特点推测最可能的方言倾向。`;
  ...
  async queryDialect(request: DialectQueryRequest): Promise<DialectQueryResult> {
    if (!this.server) {
      // 如果没有服务器，返回基于规则的简化版本
    ...
    try {
      ...
      console.error('Dialect query failed:', error);
  ...
  private buildPrompt(request: DialectQueryRequest): string {
    ...
    if (request.characterDescription) {
      prompt = prompt.replace('{{#if characterDescription}}角色描述: {{characterDescription}}{{/if}}', `角色描述: ${request.characterDescription}`);
    } else {
      prompt = prompt.replace('{{#if characterDescription}}角色描述: {{characterDescription}}{{/if}}', '');
    }

    if (request.characterBackground) {
      prompt = prompt.replace('{{#if characterBackground}}角色背景: {{characterBackground}}{{/if}}', `角色背景: ${request.characterBackground}`);
    } else {
      prompt = prompt.replace('{{#if characterBackground}}角色背景: {{characterBackground}}{{/if}}', '');
    }

    if (request.historicalPeriod) {
      prompt = prompt.replace('{{#if historicalPeriod}}历史时期: {{historicalPeriod}}{{/if}}', `历史时期: ${request.historicalPeriod}`);
    } else {
      prompt = prompt.replace('{{#if historicalPeriod}}历史时期: {{historicalPeriod}}{{/if}}', '');
    }

    if (request.region) {
      prompt = prompt.replace('{{#if region}}地区信息: {{region}}{{/if}}', `地区信息: ${request.region}`);
    } else {
  ...
  private async callLLM(prompt: string): Promise<string> {
    const request = {
      ...
      params: {
        ...
        systemPrompt: "你是一个专业的方言和语言文化专家，请准确分析角色的语言特点并返回JSON格式的结果。",
        maxTokens: 1000,
        temperature: 0.3,
      },
    };
  ...
  private generateFallbackDialect(request: DialectQueryRequest): DialectQueryResult {
    // 基于角色名称和背景的简单方言推测
